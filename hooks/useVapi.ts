...
// Create hooks/useVapi.ts: the core hook. 
// Initializes Vapi SDK, manages call lifecycle 
// (idle, connecting, starting, listening, thinking, speaking), 
// tracks messages array + currentMessage streaming, handles 
// duration timer with maxDuration enforcement, session tracking 
// via server actions
...
import { endVoiceSession, startVoiceSession } from "@/lib/actions/sessions.actions"
...
function useVapi(book: IBook) {
        ...
        const handlers = {
            'call-start': () => {
                ...
                timerRef.current = setInterval(() => {
                    if (startTimerRef.current) {
                        const newDuration = Math.floor((Date.now() - startTimerRef.current) / TIMER_INTERVAL_MS)
            ...
            'call-end': () => {
                ...
                if (sessionIdRef.current) {
                    endVoiceSession(sessionIdRef.current, durationRef.current).catch((e) => {
            ...
            message: (message: {
                ...
                if (message.transcriptType === 'final') {
                    ...
                    setMessages((prev) => {
                        const isDupe = prev.some((m) => m.role === message.role && m.content === message.transcript)
            ...
            error: (error: Error) => {
                ...
                if (sessionIdRef.current) {
                    endVoiceSession(sessionIdRef.current, durationRef.current).catch((e) => {
                        console.error('Failed to end voice session on error:', e)
                    })
                    sessionIdRef.current = null
                }
    ...
    return {
        status, isActive, messages,
        currentMessage, currentUserMessage,
        duration, start, stop, limitError,
        isBillingError, maxDurationSeconds, clearError,
